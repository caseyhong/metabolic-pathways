...
def mcf(lung_file,train_lung_ids,test_lung_ids,colon_file,train_colon_ids,test_colon_ids):
	"""
	mcf returns the final differential expressed graph for a particular cancer file 
	INPUTS: 
		lung_file - filename to parse for lung data 
		train_lung_ids - index values of which samples to take from file 
		test_lung_ids - index values of which samples to test from file 
		colon_file - filename to parse for colon data 
		train_colon_ids - index values of which samples to take from file 
		test_colon_ids - index values of which samples to test from file 
	"""
	...
	lung_patient_rxns = patientRxnMappings(lung_file,train_lung_ids,recon1RxnMappings,ensemblEntrezDict)
	lung_dg = generateDigraph(lung_patient_rxns)

	# For Colon Patients 
	print 'Processing Colon DG'
	colon_patient_rxns = patientRxnMappings(colon_file,train_colon_ids,recon1RxnMappings,ensemblEntrezDict)
...
if __name__ == '__main__':
	...
	diff_g, lung_test, colon_test = mcf(lung_file,lung_50[:40],lung_50[-10:],colon_file,colon_50[:40],colon_50[-10:])
